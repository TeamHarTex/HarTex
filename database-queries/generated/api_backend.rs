...
#[allow(non_camel_case_types)] pub enum BorsRepositoryPermissions { review,r#try,}impl < 'a > postgres_types :: ToSql for BorsRepositoryPermissions
{
    fn
    ...
    {
        let s = match * self { BorsRepositoryPermissions :: review => "review",BorsRepositoryPermissions :: r#try => "try",}
        ; buf.extend_from_slice(s.as_bytes()) ; std :: result :: Result ::
        Ok(postgres_types :: IsNull :: No)
    } fn accepts(ty : & postgres_types :: Type) -> bool
    {
        if ty.name() != "bors_repository_permissions" { return false ; } match * ty.kind()
        {
            postgres_types :: Kind :: Enum(ref variants) =>
            {
                if variants.len() != 2 { return false ; }
                variants.iter().all(| v | match & * * v
                { "review" => true,"try" => true,_ => false, })
            } _ => false,
        }
    } fn
...
} impl < 'a > postgres_types :: FromSql < 'a > for BorsRepositoryPermissions
{
    ...
    BorsRepositoryPermissions, Box < dyn std :: error :: Error + Sync + Send >, >
    {
        match std :: str :: from_utf8(buf) ?
        {
            "review" => Ok(BorsRepositoryPermissions :: review),"try" => Ok(BorsRepositoryPermissions :: r#try),s => Result ::
...
{ use futures::{{StreamExt, TryStreamExt}};use futures; use cornucopia_async::GenericClient;#[derive( Debug)] pub struct SelectUserByRepositoryAndPermissionsParams < T1 : cornucopia_async::StringSql,T2 : cornucopia_async::ArraySql<Item = super::super::types::public::BorsRepositoryPermissions>,> { pub repository : T1,pub permission_array : T2,}pub struct StringQuery < 'a, C : GenericClient, T, const N : usize >
{
    client : & 'a  C, params :
...
}pub fn select_user_by_repository_and_permissions() -> SelectUserByRepositoryAndPermissionsStmt
{ SelectUserByRepositoryAndPermissionsStmt(cornucopia_async :: private :: Stmt :: new("SELECT
    \"username\"
FROM
...
WHERE
    \"repository\" = $1 AND
    \"permissions\" && $2")) } pub
struct SelectUserByRepositoryAndPermissionsStmt(cornucopia_async :: private :: Stmt) ; impl
SelectUserByRepositoryAndPermissionsStmt { pub fn bind < 'a, C : GenericClient, T1 : cornucopia_async::StringSql,T2 : cornucopia_async::ArraySql<Item = super::super::types::public::BorsRepositoryPermissions>,>
(& 'a mut self, client : & 'a  C,
repository : & 'a T1,permission_array : & 'a T2,) -> StringQuery < 'a, C,
String, 2 >
{
    StringQuery
    {
        client, params : [repository,permission_array,], stmt : & mut self.0, extractor :
        | row | { row.get(0) }, mapper : | it | { it.into() },
    }
} }impl < 'a, C : GenericClient, T1 : cornucopia_async::StringSql,T2 : cornucopia_async::ArraySql<Item = super::super::types::public::BorsRepositoryPermissions>,> cornucopia_async ::
Params < 'a, SelectUserByRepositoryAndPermissionsParams < T1,T2,>, StringQuery < 'a,
C, String, 2 >, C > for SelectUserByRepositoryAndPermissionsStmt
{
    fn
    params(& 'a mut self, client : & 'a  C, params : & 'a
    SelectUserByRepositoryAndPermissionsParams < T1,T2,>) -> StringQuery < 'a, C,
    String, 2 >
...
{ pub component : String,pub timestamp : time::OffsetDateTime,}pub struct SelectStartTimestampByComponentBorrowed < 'a >
{ pub component : &'a str,pub timestamp : time::OffsetDateTime,} impl < 'a > From < SelectStartTimestampByComponentBorrowed <
'a >> for SelectStartTimestampByComponent
{
    fn
    from(SelectStartTimestampByComponentBorrowed { component,timestamp,} : SelectStartTimestampByComponentBorrowed < 'a >)
...
}pub struct SelectStartTimestampByComponentQuery < 'a, C : GenericClient, T, const N : usize >
{
    client : & 'a  C, params :
    [& 'a (dyn postgres_types :: ToSql + Sync) ; N], stmt : & 'a mut cornucopia_async
    :: private :: Stmt, extractor : fn(& tokio_postgres :: Row) -> SelectStartTimestampByComponentBorrowed,
...
where C : GenericClient
{
    pub fn map < R > (self, mapper : fn(SelectStartTimestampByComponentBorrowed) -> R) -> SelectStartTimestampByComponentQuery
    < 'a, C, R, N >
    {
...
WHERE
    \"component\" = $1")) } pub
struct SelectStartTimestampByComponentStmt(cornucopia_async :: private :: Stmt) ; impl
SelectStartTimestampByComponentStmt { pub fn bind < 'a, C : GenericClient, T1 : cornucopia_async::StringSql,>
(& 'a mut self, client : & 'a  C,
...
ON CONFLICT (\"component\") DO UPDATE
    SET
        \"timestamp\" = $2")) } pub
struct StartTimestampUpsertStmt(cornucopia_async :: private :: Stmt) ; impl
StartTimestampUpsertStmt { pub async fn bind < 'a, C : GenericClient, T1 : cornucopia_async::StringSql,>
(& 'a mut self, client : & 'a  C,
...
cornucopia_async :: Params < 'a, StartTimestampUpsertParams < T1,>, std::pin::Pin<Box<dyn futures::Future<Output = Result <
u64, tokio_postgres :: Error > > + Send + 'a>>, C > for StartTimestampUpsertStmt
{
    fn
    params(& 'a mut self, client : & 'a  C, params : & 'a
    StartTimestampUpsertParams < T1,>) -> std::pin::Pin<Box<dyn futures::Future<Output = Result < u64, tokio_postgres ::
    Error > > + Send + 'a>> { Box::pin(self.bind(client, & params.component,& params.timestamp,) ) }
}}}