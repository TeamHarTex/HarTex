...
crate struct RoleGlobalRemoveCommand;

impl Command for RoleGlobalRemoveCommand {
    fn fully_qualified_name(&self) -> String {
        String::from("role global-remove")
    }
    ...
    fn execute_command<'asynchronous_trait>(ctx: CommandContext<'asynchronous_trait>,
        ...
        Box::pin(administrator_role_global_remove_command(ctx, role_id.to_string(), cache))
    }
...
async fn administrator_role_global_remove_command(ctx: CommandContext<'_>, role_id: String, cache: InMemoryCache)
                                               -> SystemResult<()> {
    ...
    if let Ok(rid) = role_id.parse() {
        ...
        ctx.http_client.clone().create_message(channel_id)
            .content(format!("<:green_check:705623382682632205> Removed role `{}` from every member in the guild..", role.mention()))?
            .allowed_mentions().replied_user(false).build()
            .reply(ctx.message.id)
            .await?;
