...
pub(crate) const DO_NOT_MODIFY_HEADER: &str = "// ==================! DO NOT MODIFY !==================
    ...
    P: AsRef<Path>,
{
    ...
    let _ = schemas
        ...
        .process_results(|iter| iter.map(|file| {
            let path = tables_dir.clone().join(file.filename);

            fs::write(&path, DO_NOT_MODIFY_HEADER.to_owned() + file.content.as_str())?;
